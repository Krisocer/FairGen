...
def sample_by_race_proportion(race_disease_dict, num, race_proportions):
    ...
    # 按种族比例来选择样本
    # - race_disease_dict: {race: {disease: [sample_indices]}}
    # - num: 要选取的总样本数
    # - race_proportions: {race: proportion}，每个种族的比例
    ...
    # 为了保证每个疾病的样本数目均衡，先计算每个疾病要选取的样本数
    ...
    # 从每个疾病中采样
        for disease, indices in disease_dict.items():
            if len(indices) >= samples_per_disease:
                ...
                selected_indices.extend(indices)  # 如果样本不足，全部选入
...
for seed in [3]:
    ...
    for num in range(100,900,100):
        ...
        model.fc = torch.nn.Sequential(
            ...
            torch.nn.Linear(num_features, 2)  # 修改为你的类别数
        )
