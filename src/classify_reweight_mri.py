...
parser.add_argument("--best_worst", action="store_true", help="Save best and worst models for each seed+num combination")

# Parse the arguments
args = parser.parse_args()

# Create model_for_umap directory if best_worst is enabled
if args.best_worst:
    model_save_dir = "/ocean/projects/ccr200024p/zli27/sd_xray/sd/src/model_for_umap"
...
for seed in range(3,7,1):
    ...
    for num in range(100,900,100):
        ...
        if args.aug_data:
            ...
            selected_indices = sample_by_race_proportion(race_disease_dict, num * 4, race_proportions)
        ...
        # Modify the classifier for better performance
        ...
        # Track models for epoch 5 and final epoch if best_worst flag is enabled
        epoch_5_model_state = None
        final_model_state = None

        for epoch in range(args.epochs):
            # If it's the first epoch, use uniform class weights
            if epoch == 0:
                class_weights = torch.ones(2)  # Assuming 3 races
            else:
            ...
            # Save epoch 5 and final epoch models if best_worst flag is enabled
            if args.best_worst:
                if epoch == 4:  # epoch 5 (0-indexed)
                    epoch_5_model_state = {k: v.cpu() if isinstance(v, torch.Tensor) else v for k, v in model.state_dict().items()}
                    print(f"Epoch 5 model state saved for seed{args.seed}_num{num}")
                
                if epoch == args.epochs - 1:  # final epoch
                    final_model_state = {k: v.cpu() if isinstance(v, torch.Tensor) else v for k, v in model.state_dict().items()}
                    print(f"Final epoch model state saved for seed{args.seed}_num{num}")
        ...
        if args.best_worst:
            ...
            # Save epoch 5 model
            if epoch_5_model_state is not None:
                epoch_5_model_name = f"mri_epoch5_seed{args.seed}_num{num}"
                epoch_5_model_path = os.path.join(model_save_dir, epoch_5_model_name)
                os.makedirs(epoch_5_model_path, exist_ok=True)
                model.load_state_dict(epoch_5_model_state)
                model.save_pretrained(epoch_5_model_path)
                print(f"Epoch 5 model saved to {epoch_5_model_path}")
            
            # Save final epoch model
            if final_model_state is not None:
                final_model_name = f"mri_final_seed{args.seed}_num{num}"
