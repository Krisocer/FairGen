...
def main():
    ...
    if args.modality == "skin":
        num_class = 15  # 3 Attributes * 5 Diseases
    elif args.modality == "mri":
        num_class = 4   # 2 Attributes * 2 Diseases
    elif args.modality == "chest":
        num_class = 10  # 2 Attributes * 5 Diseases
    else:
    ...
    generated_image_dir = "/ocean/projects/ccr200024p/zli27/sd_xray/output/sd_skin/cbcb"  

    image_data = defaultdict(list)

    for folder_name in os.listdir(generated_image_dir):
        folder_path = os.path.join(generated_image_dir, folder_name)
        ...
        for file_name in os.listdir(folder_path):
            if file_name.endswith((".png", ".jpg", ".jpeg")):  
                ...
                if "Demented" in words:
                    words.remove("Demented")
                    words.append("demented")  
                if "Non-demented" in words:
                    words.remove("Nondemented")
                    words.append("nondemented")  


                prompt = " ".join(words).lower()


                image_data[prompt].append(image_path)

    print("data is from: ", generated_image_dir)
    # 打印检查
    for prompt, images in image_data.items():
        print(f"Prompt: {prompt} -> {len(images)} images loaded.")

    '''
    def calculate_similarity(image_1, image_2, prompt):
        images = [Image.open(image_1).convert("RGB"), Image.open(image_2).convert("RGB")]
        inputs = clip_processor(text=[prompt, prompt], images=images, return_tensors="pt", padding=True).to(accelerator.device)

        with torch.no_grad():
            image_features = clip_model.get_image_features(pixel_values=inputs["pixel_values"])
            text_features = clip_model.get_text_features(input_ids=inputs["input_ids"])

        similarity_1 = torch.cosine_similarity(image_features[0].unsqueeze(0), text_features).mean().cpu().numpy()
        similarity_2 = torch.cosine_similarity(image_features[1].unsqueeze(0), text_features).mean().cpu().numpy()


        return similarity_1, similarity_2


    def create_image_pair(prompt, image_data):
        image_pair = random.sample(image_data[prompt], 2)
        image_1, image_2 = image_pair[0], image_pair[1]

        similarity_1, similarity_2 = calculate_similarity(image_1, image_2, prompt)

        if similarity_1 > similarity_2:
            return image_1, image_2  # 选择第一张为优选
        else:
            return image_2, image_1  # 选择第二张为优选
        
    # 加载 CLIP 模型用于 DPO 训练
    clip_processor = CLIPProcessor.from_pretrained("openai/clip-vit-base-patch16")
    clip_model = CLIPModel.from_pretrained("openai/clip-vit-base-patch16").to(accelerator.device)
    clip_model.eval()
    '''
    ...
    def compute_class_weights(dataset, label_column):
        ...
        # 计算每个类别的样本数量
        class_counts = np.bincount(all_labels_encoded)

        # 计算类别权重
        class_weights = 1.0 / class_counts
        sampling_weights = class_weights[all_labels_encoded]
        return sampling_weights

    # 获取训练集的样本权重
    sampling_weights = compute_class_weights(dataset["train"], "text")  # 假设标签列名为 'label'

    # 创建 WeightedRandomSampler
    sampler = WeightedRandomSampler(sampling_weights, num_samples=len(sampling_weights), replacement=True)

    # DataLoaders creation with sampler:
    train_dataloader = torch.utils.data.DataLoader(
        train_dataset,
        sampler=sampler,
        collate_fn=collate_fn,
...
#     weight = class_counter(train_dataset.targets)
#     print(weight)
    ...
    # classweight = class_counter(dataset["train"], "text")
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                def generateinput_ids(prompt):
                    ...
                    # 使用与 tokenize_captions 相同的 tokenizer 设置
                    inputs = tokenizer(
                        prompt, 
                ...
                if args.snr_gamma is None:
                    # loss_ddpm = F.mse_loss(model_pred.float(), target.float(), reduction="mean")
                    
                    # loss_reg = loss_com = torch.tensor(0).to(noisy_latents.device)
                    ...
                    y_bal = torch.Tensor(np.random.choice(
                                             ...
                                             )).to(noisy_latents.device).long()

                    # print(y_bal)
                    labels_str = [label_mapping[int(label.item())] for label in y_bal]
                    # print(labels_str)

                    input_id2 = generateinput_ids(labels_str).to(noisy_latents.device)
                    ...
                    loss_sum = loss_reg + 1/4 * loss_com
                    loss_sum = loss_sum.mean()
                    loss_ddpm = loss_ddpm.mean()
                        
                    loss = loss_ddpm + loss_sum if loss_sum > 0 else loss_ddpm
